...
def get_channel_profile_image(channel_id):
    search_url = base_channel_url + 'part=snippet&id={}&fields=items/snippet/thumbnails&key={}'.format(channel_id,
                                                                                                       api_key)
    inp = urllib.request.urlopen(search_url)
    resp = json.load(inp)
...
def get_newest_video_in_channel(channel_id):
    search_url = base_search_url + 'channelId={}&order=date&part=id&type=video&maxResults=1&key={}'.format(channel_id,
                                                                                                           api_key)
    inp = urllib.request.urlopen(search_url)
    resp = json.load(inp)
