...
class Commands(commands.Cog):
    ...
    async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="Created",
            value=ctx.guild.created_at.strftime("%a, %d %b %Y \n %H:%M:%S %ZGMT"),
        )
        ...
        for channel in ctx.guild.channels:
            if isinstance(channel, discord.TextChannel):
                ...
                voice_channels += 1
        embed.add_field(
            name="Channels", value=f"{text_channels} text, {voice_channels} voice"
        )
        ...
        embed.add_field(
            name="Explicit Content Filter", value=ctx.guild.explicit_content_filter
        )
    ...
    async def botinfo(self, ctx):
        ...
        embed.add_field(
            name="Bot Created",
            value=self.bot.user.created_at.strftime("%a, %d %b %Y \n %H:%M:%S %ZGMT"),
        )
    ...
    async def invite(self, ctx):
        await ctx.send(
            f"Invite me to your server: {discord.utils.oauth_url(self.bot.user.id)}"
        )
    ...
    async def timedelta(self, ctx, timezone1: str, timezone2: str, *, time: str):
        ...
        embed.add_field(
            name="Time",
            value=f'{time2.strftime("%Y-%m-%d %I:%M %p %Z")} is {time3.strftime("%Y-%m-%d %I:%M %p")} in {timezone2}',
        )
        await ctx.send(embed=embed)
    ...
    async def hello(self, ctx):
        ...
        await ctx.send("Hi", view=view, embed=embed)
